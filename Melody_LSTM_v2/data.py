...
class DataLoader:
    ...
    def __init__(self,
        ...
        """
        if max_midi_num = None, it will get all the midi files in the given dir,
        or will only get in max_midi_num midi files
    ...
    def split_transpose(self, Split_Interval):
        ...
        for file in files:
            i += 1
            if self.max_midi_num is not None and i > self.max_midi_num:
                break
            ...
            for element in notes:
                ...
                if isinstance(element, music21.note.Note):
                    step = getStep(element.pitch.ps, tonicID,
                                   key.name.split(" ")[1])
                    Sequence[index] = MyNote(step, element.quarterLength,
                                             element.volume.getRealized())
            ...
            for note in seq:
                ...
                target_features = torch.tensor([
                    ...
                    getDurationIndex(note.duration), note.volume
                                               ...
                                               dtype=torch.float32)
                target.append(
                    target_features.unsqueeze(dim=0).unsqueeze(dim=-1))
