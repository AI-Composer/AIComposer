    ...
    def train_model(self, dataLoader):
        ...
        for epoch in range(epoches):
            for batch_idx in range(len(batches)):
                ...
                target_p = target[1:, 0].view((L - 1, )).to(device,
                                                            dtype=torch.int64)
                target_d = target[1:, 1].view((L - 1, )).to(device,
                                                            dtype=torch.int64)
                target_v = target[1:, 2].view((L - 1, )).to(device,
                                                            dtype=torch.int64)

                Loss = F.cross_entropy(pitch, target_p) + F.cross_entropy(
                    duration, target_d)  # + F.mse_loss(volume, target_v)
                Loss.backward()
                self.optimizer.step()
                if batch_idx % 10 == 0:
                    print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.4f}'.
                          format(epoch, batch_idx, len(batches),
